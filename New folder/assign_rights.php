...
$count_arraya 		= $_POST['count_arraya'];
		...
		$reg=$arr2[1];
		$sql_pstt="insert into rs_tbl_user_rights(user_cd,div_cd,region_cd,count_cd,project_cd,user_type)
VALUES ($uid,$div,$reg,NULL,NULL,$user_type)";
...
$country_arr 		= $_POST['country_arr'];
		...
		$cont=$arr2[2];
		$sql_pstt="insert into rs_tbl_user_rights(user_cd,div_cd,region_cd,count_cd,project_cd,user_type)
VALUES ($uid,$div,$reg,$cont,NULL,$user_type)";
$res_pstt=mysqli_query($con,$sql_pstt);	
		}
		header("Location:manage_user.php");
 }
...
?>	
		<table width="100%" border="1px solid black" cellspacing="0px" bgcolor="#FFF">
			<tr height="30px" bgcolor="#DADADA"><td width="25%" style="font-weight:bold">Division</td><td width="20%"  style="font-weight:bold">Region</td><td width="55%"  style="font-weight:bold">Country</td><td width="55%"  style="font-weight:bold">Project</td></tr>   <!--//// END tbl header-->
<?PHP
			...
			<td width="25%">
			<?php 
			$Sqlcc1= "SELECT * FROM rs_tbl_user_rights where user_cd='$uid' and user_type='$user_type' and div_cd='$divid' group by div_cd";
			...
			?>
			<input type="checkbox" name="cmid_<?php echo $divid?>" id="cmid_<?php echo $divid?>" value="<?php echo $divid; ?>"  onclick="getcommid_p(<?php echo $divid; ?>)" <?php if($checked9=="1"){?>checked<?php }?>/> <?=$result["dname"]; ?><?php 
	 ...
	 $Sqlcc6= "SELECT * FROM rs_tbl_user_rights where user_cd='$uid' and user_type='$user_type' and div_cd='$divid' group by div_cd";
...
FROM
    rs_tbl_user_rights
    INNER JOIN ds002region 
        ON (rs_tbl_user_rights.region_cd = ds002region.rid) where div_cd='$div_id' and user_cd='$uid' and user_type='3' and region_cd='$cidd' group by region_cd";
...
$div_id=$res21['division'];
			...
			$countryid=$res21['country'];
				$Sqlcte = "SELECT * FROM rs_tbl_user_rights where user_cd='$uid' and user_type='$user_type' and div_cd=$div_id and region_cd=$region_id";
			$rescte=mysqli_query($con,$Sqlcte);
			if(mysqli_num_rows($rescte)>0)
			{
				$Sqlct = "SELECT * FROM rs_tbl_user_rights where user_cd='$uid' and user_type='$user_type' and div_cd=$div_id and region_cd=$region_id and count_cd=$countryid ";
...
<input type="checkbox"  name="cmid_<?php echo $div_id?>_<?php echo $region_id; ?>_<?php echo $countryid; ?>"  id="cmid_<?php echo $div_id?>_<?php echo $region_id; ?>_<?php echo $countryid; ?>" value="<?php echo $countryid; ?>"  onchange="getproject_p(<?php echo $div_id?>,<?php echo $region_id; ?>_<?php echo $countryid; ?>)" <?php if($checkedp1=="1"){?>checked<?php }?>/> <?php			
...
?>
            ...
            <table border="0" width="100%">
			<?php 
			$sql_lis35="select region from saca_project_master where division=$divid and region=$rid group by region";
			...
			echo $sql_lis31="select master_code,region,division, country, project_desc from saca_project_master where division=$divid and region=$rid and country=$conid";
				 $res_lis31=mysqli_query($con,$sql_lis31);
...
$div_id=$res21['division'];
			...
			$master_code=$res21['master_code'];
				$Sqlcte = "SELECT * FROM rs_tbl_user_rights where user_cd='$uid' and user_type='$user_type' and div_cd=$div_id and region_cd=$region_id and count_cd=$countryid";
			$rescte=mysqli_query($con,$Sqlcte);
			if(mysqli_num_rows($rescte)>0)
			{
				$Sqlct = "SELECT * FROM rs_tbl_user_rights where user_cd='$uid' and user_type='$user_type' and div_cd=$div_id and region_cd=$region_id and count_cd=$countryid and master_code=$master_code";
			$resct=mysqli_query($con,$Sqlct);
					$resultct=mysqli_fetch_assoc($resct);
					if(mysqli_num_rows($resct)>0)
						{
